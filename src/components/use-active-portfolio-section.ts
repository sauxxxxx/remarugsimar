  ...
  useEffect(() => {
    ...
    function updateActiveSection() {
      const panelHasIndependentScroll =
        contentPanel !== null &&
        ["auto", "scroll"].includes(window.getComputedStyle(contentPanel).overflowY);
      const panelTop = contentPanel?.getBoundingClientRect().top ?? 0;
      const scrollPosition = panelHasIndependentScroll ? contentPanel.scrollTop : window.scrollY;
      ...
      let nextSection: PortfolioSectionId = portfolioSections[0].id;
